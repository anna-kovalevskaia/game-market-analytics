CREATE TABLE IF NOT EXISTS raw_dq.metrics_status (
    ...
    row_hash UInt64 MATERIALIZED cityHash64(ifNull(toString(dag_id), '\\N'), ifNull(toString(schema_name), '\\N'), ifNull(toString(table_name), '\\N'), ifNull(toString(metrics_name), '\\N'), ifNull(toString(agg_type), '\\N'), ifNull(toString(metrics_value), '\\N')),
