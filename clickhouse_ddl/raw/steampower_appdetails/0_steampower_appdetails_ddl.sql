CREATE TABLE IF NOT EXISTS raw.steampower_appdetails (
    ...
    row_hash UInt64 MATERIALIZED cityHash64(ifNull(toString(success), '\\N'), ifNull(toString(appid), '\\N'), ifNull(toString(name), '\\N'), ifNull(toString(required_age), '\\N'), ifNull(toString(is_free), '\\N'), ifNull(toString(dlc), '\\N'), ifNull(toString(supported_languages), '\\N'), ifNull(toString(website), '\\N'), ifNull(toString(pc_requirements), '\\N'), ifNull(toString(mac_requirements), '\\N'), ifNull(toString(linux_requirements), '\\N'), ifNull(toString(developers), '\\N'), ifNull(toString(publishers), '\\N'), ifNull(toString(categories_id), '\\N'), ifNull(toString(categories_description), '\\N'), ifNull(toString(genres_id), '\\N'), ifNull(toString(genres_description), '\\N'), ifNull(toString(release_date), '\\N')),
