CREATE TABLE IF NOT EXISTS raw.steampower_packages (
    ...
    row_hash UInt64 MATERIALIZED cityHash64(ifNull(toString(appid), '\\N'), ifNull(toString(name), '\\N'), ifNull(toString(packageid), '\\N'), ifNull(toString(package_option_text), '\\N'), ifNull(toString(package_price_with_discount), '\\N')),
