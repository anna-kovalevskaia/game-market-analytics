...
from data_models.steampower_appid import TableConfig
from data_models.metrics_status import MetricsStatusModel
...
def steamappid_extract() -> str:
    ...
    params = {"delay_seconds": 1.3, "count": 100, "sort_by":"Released_DESC", "max_rows": 2000}
...
@task(outlets=[table_asset(TableConfig)])
def steamappid_insert_to_clickhouse(run_id_path: str) -> None:
    ...
    steamappid_parquet_to_clickhouse(
        ...
        batch_size=2000,
        raw=TableConfig,
        raw_dq=MetricsStatusModel,
        check=Check({"COUNT_METRIC":"row_count"}),
        cur_date=cur_date,
    )


@dag(
    dag_id="steamappid_raw_data_daily",
    schedule="30 0 * * 1-6",
