...
from dags_utils.operations.steampower_apptag_ops import (
    get_appids_to_fetch,
    steamapptag_extract_to_tmp,
    steamapptag_parquet_to_clickhouse,
)
...
from data_models.steampower_apptag import TableConfig as SteamPowerAppTagTable


@task
def steamapptag_extract() -> str:
    ...
    params = {"delay_seconds": 3, "appids": appids, "batch_size": 200}
    steamapptag_extract_to_tmp(client=steampower_client, run_id_path=run_id_path, **params)

    return str(run_id_path)


@task(outlets=[table_asset(SteamPowerAppTagTable)])
def steamapptag_insert_to_clickhouse(run_id_path: str) -> None:
...
@dag(
    dag_id="steamapptag_raw_data",
    schedule="0 */4 * * *",
    start_date=datetime(2026, 1, 1),
    catchup=False,
    max_active_runs=1,
)
def steamapptag__process():
    tmp_dir = steamapptag_extract()
    steamapptag_insert_to_clickhouse(run_id_path=tmp_dir)


steamapptag__process()
