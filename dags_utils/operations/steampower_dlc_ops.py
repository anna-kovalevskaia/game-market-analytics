...
from data_models.steampower_dlc_details import SteamPowerDlcDetailsModel
from data_models.steampower_dlc_details import TableConfig as SteamPowerDlcDetailsTable
from data_models.steampower_dlc_packages import SteamPowerDlcPackagesModel
from data_models.steampower_dlc_packages import TableConfig as SteamPowerDlcPackagesTable
from data_models.steampower_dlc_price import SteamPowerDlcPriceModel
from data_models.steampower_dlc_price import TableConfig as SteamPowerDlcPriceTable

logger = logging.getLogger(__name__)

# The order of the elements must match the result in steampower_get_dlc.
STREAMS: tuple[tuple[type[BaseModel], type], ...] = (
    (SteamPowerDlcDetailsModel, SteamPowerDlcDetailsTable),
    (SteamPowerDlcPriceModel, SteamPowerDlcPriceTable),
    (SteamPowerDlcPackagesModel, SteamPowerDlcPackagesTable),
)
...
def get_appids_to_fetch(ch_client: ClickHouseClient) -> list[int] | None:
    table = ch_client.sql_to_arrow("SELECT dlc_appid FROM meta.appid_fetch_dlc")
    if not table:
        logger.warning("No appids to fetch.")
        return None
    return table["dlc_appid"].to_pylist()


def _steamdlc_write_to_tmp(
    data: list[BaseModel],
    model: type[BaseModel],
    full_file_path: Path,
) -> None:
    """Write one Steam dlc stream to a temporary parquet file."""
...
def steamdlc_extract_to_tmp(client: SteamPowerClient, run_id_path: Path, **kwargs) -> None:
    ...
    pages = client.steampower_get_dlc(
    ...
    for page_num, page_rows in enumerate(pages):
        for rows, (model, table) in zip(page_rows, STREAMS, strict=True):
            ...
            _steamdlc_write_to_tmp(validate_result, model, full_file_path)
            logger.info("SteamPower written %s to %s", page_num, table.table_name)


def steamdlc_metrics_validate(
